<template>
  ...
  <span
    v-else-if="!!title"
    :class="['lc-title', className, { 'has-tip': false, 'only-icon': !data._title.label }]"
    @click="handleClick"
  >
...
import { IPublicTypeI18nData, IPublicTypeTitleConfig, IPublicTypeTitleProps } from '@cc/lowcode-types'
import { intl } from '../../intl'
import { createIcon, isTitleConfig } from '@cc/lowcode-utils';
...
const renderLabel = (label: string | IPublicTypeI18nData | VNode) => {
  ...
  if (match && keywords) {
    ...
    labelToRender = fragments.map((f: string) => h('span', { style: { color: f === keywords ? 'red' : 'inherit' }}))
  }

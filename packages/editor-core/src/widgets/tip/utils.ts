...
export function resolvePosition(popup: any, target: any, arrow: any, bounds: any, prefer: any) {
  ...
  if (dir === 'top' || dir === 'bottom') {
    ...
    arrowLeft = Math.min(
      popup.width - arrow.width,
      Math.max(target.left - (arrow.width - target.width) / 2 - left, 0)
    );
  } else {
    if (dir === 'left') {
    ...
    arrowTop = Math.min(
      popup.height - arrow.height,
      Math.max(target.top - (arrow.width - target.height) / 2 - top, 0)
    );
  }
